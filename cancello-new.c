...
int main (int argc, char ** argv) {
  ...
  while (1) {
    ...
    if (modbus_connect(mb_zbrn1)==-1) { // CONNESSIONE
      sprintf(errmsg,"ERRORE Connessione ZBRN1 [%s], WAIT 5 sec.\n",modbus_strerror(errno));
      logvalue(LOG_FILE,errmsg);
      sleep(5);
    } else  /*ELSE CONNESSIONE OK*/ { 
      if ( modbus_read_registers(mb_zbrn1, 0, 1, zbrn1_reg) < 0 ) { // LETTURA REGISTRI
	// leggo lo stato degli ingressi collegati al wireless button
	sprintf(errmsg,"\tERRORE Lettura Registro ZBRN1 per Cancello [%s], WAIT 1 sec. \n",modbus_strerror(errno));
	logvalue(LOG_FILE,errmsg);
	sleep(1);
      } else /*ELSE LETTURA REGISTRI OK*/ { 
